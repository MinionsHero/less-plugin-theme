            ...
            case '}':
                if (themeSyntaxAppear) {
                    ...
                    if (lastThemeSyntax && lastThemeSyntax.end === index) {
                        ...
                        throw new Error('匹配了theme({，但是未找到后面的}')
                    }
                }
...
module.exports = function parser(input) {
    let temp = init(input)
    const commentArr = lookForComment(temp)
    let str = ''
